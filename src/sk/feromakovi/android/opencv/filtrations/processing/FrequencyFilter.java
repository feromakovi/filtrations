...
public final class FrequencyFilter implements ImageProcessor {
	...
	public Bitmap process(Bitmap image) {
		...
		Mat processedMat = computeDFT(mat);
	...
	private Mat computeDFT(Mat image) {
//		int m = Core.getOptimalDFTSize(image.rows());
//		int n = Core.getOptimalDFTSize(image.cols());
//
//		Mat padded = new Mat(new Size(n, m), CvType.CV_32F);
//		Imgproc.copyMakeBorder(image, padded, 0, m - image.rows(), 0, n - image.cols(), Imgproc.BORDER_CONSTANT);
//		List<Mat> planes = new ArrayList<Mat>();
//		planes.add(padded);
//		planes.add(Mat.zeros(padded.rows(), padded.cols(), CvType.CV_32F));
//		Mat complex = Mat.zeros(padded.rows(), padded.cols(), CvType.CV_32F);
//		Core.merge(planes, complex); 
//		Core.dft(complex, complex, Core.DFT_COMPLEX_OUTPUT, 0);
//		return complex;
		image.convertTo(image, CvType.CV_64FC1);

	    int m = Core.getOptimalDFTSize(image.rows());
	    int n = Core.getOptimalDFTSize(image.cols()); // on the border
	                                                    // add zero
	                                                    // values
	    ...
	    Imgproc.copyMakeBorder(image, padded, 0, m - image.rows(), 0,
	            n - image.cols(), Imgproc.BORDER_CONSTANT);
